...
(function () {
  ...
  function buildReport() {
    ...
    if (state.discover && state.discover.results) {
      lines.push('## 3. 相似内容直达 / Similar-content direct results');
      lines.push('');
      ...
      for (const k of ['image', 'video', 'article', 'wiki']) {
        ...
        for (const it of groups[k]) {
          ...
          let host = '';
          try { host = new URL(it.url).hostname; } catch { /* ignore */ }
          lines.push(`${m}. ${it.title || host}. ${host}. <${it.url}> (retrieved ${now.toISOString().slice(0, 10)}, via ${it.engine})`);
        }
        lines.push('');
      }
    }
  ...
  // ------------------------------------------------------------
  // 相似内容直达结果（v1.3：关键词 → 服务器抓取真实网址）
  // ------------------------------------------------------------
  async function runDiscover(keywords) {
    if (!state.health || !state.health.ok) { toast(t('serverDown')); return; }
    const kws = (keywords || []).filter(Boolean).slice(0, 2);
    if (!kws.length) { toast(t('noImage')); return; }
    $('#sec-discover').hidden = false;
    $('#disc-status').textContent = t('discBusy');
    $('#disc-kw').textContent = `🔍 ${kws.join('  ·  ')}`;
    $('#disc-list').innerHTML = '';
    try {
      ...
      if (!resp.ok) throw new Error(await resp.text());
      state.discover = await resp.json();
      state.discoverTab = 'all';
      renderDiscover();
      const items = Object.values(state.discover.results || {}).reduce((a, r) => a + r.items.length, 0);
      const okCount = Object.values(state.discover.results || {}).filter((r) => r.items.length).length;
      $('#disc-status').textContent = t('discDone')(items, okCount) + ` · ${new Date(state.discover.ts).toLocaleTimeString()}`;
    } catch (e) {
      $('#disc-status').textContent = String(e.message || e).slice(0, 90);
    }
  }
  ...
  function renderDiscover() {
    ...
    $('#disc-tabs').innerHTML = ['all', ...kinds]
      .filter((k) => k === 'all' || counts[k] > 0)
      .map((k) => `<button class="tab ${state.discoverTab === k ? 'active' : ''}" data-tab="${k}">${t('discTabs')[k]}<span class="cnt">${counts[k]}</span></button>`)
      .join('');

    const shown = all.filter((x) => state.discoverTab === 'all' || x.kind === state.discoverTab);
    if (!shown.length) {
    ...
    $('#disc-list').innerHTML = shown.map((it) => {
      ...
      const thumb = it.thumb
        ? `<img class="r-thumb" src="${esc(it.thumb)}" alt="" loading="lazy" onerror="this.style.visibility='hidden'">`
        : `<div class="r-thumb placeholder">${kindIcon[it.kind] || '•'}</div>`;
      return `
      <div class="result-item">
        ${thumb}
        <div class="r-main">
          ...
          <div class="r-url">${esc(it.url)}</div>
          <div class="r-badges">
            <span class="badge">${t('discTabs')[it.kind] || it.kind}</span>
            <span class="badge">${it.engine}</span>
            ${it.source ? `<span class="badge">${esc(String(it.source).slice(0, 30))}</span>` : ''}
          </div>
        </div>
      </div>`;
    }).join('');
  ...
  $('#disc-tabs').addEventListener('click', (ev) => {
    const tab = ev.target.closest('.tab');
    if (!tab) return;
    state.discoverTab = tab.dataset.tab;
    renderDiscover();
  });

  $('#btn-disc-refresh').addEventListener('click', () => {
    const kw = $('#keyword-input').value.trim();
  ...
  $('#btn-recognize').addEventListener('click', async () => {
    ...
    try {
      ...
      state.lastRec = res;
      renderRecognize(res);
      // 自动抓取相似内容的直达网址（识别出的检索词，最多 2 个）
      runDiscover((state.recQueries || []).slice(0, 2));
    } catch (e) {
