...
### H. 社区帖子源与零点击流程（v1.4，2026-09-19 实测）

| 声明 | 核验方法与证据 | 结论 |
| --- | --- | --- |
| Hacker News Algolia API 免密钥可用 | 服务端请求 `hn.algolia.com/api/v1/search` 返回 200 与 5 条真实帖子（含外链 URL 与 HN item 链接） | ✅ 采用 |
| Reddit `search.json` 可用 | 本机代理出口下超时（15s）；接口本身为 Reddit 官方公开端点，多数网络可用 | ⚠️ 尽力而为（失败自动降级，不影响其它源） |
| 微博 `s.weibo.com` 搜索可用 | 服务端请求 200 但 9KB 登录墙（含"登录"），无未登录内容 | ❌ 不采用 |
| 载图零点击自动检索 | 真实浏览器 E2E：刷新页面后**唯一动作是载入图片 URL** → 模型静默恢复（`pt-clip-auto` 记忆 + 浏览器缓存权重）→ 自动识别"商品图 38.1%"→ 关键词自动填入"商品 同款"→ **123 条直达网址自动出现**（相似图片 71 / 相关视频 20 / 文章网页 16 / 社区帖子 8 / 百科词条 8，7 源命中），引擎区默认折叠；截图已更新 | ✅ |
| HN 帖子条目真实性 | 页内样本：`simonwillison.net/2024/Sep/15/how-to-succeed-in-mrbeast-production/`（2115 points）等，均为 HN 真实热帖外链 | ✅ |
