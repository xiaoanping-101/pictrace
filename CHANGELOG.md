...
## [1.3.0] — 2026-09-19（相似内容直达结果：页内直接给出真实网址）

### 新增

- **`/api/discover` 端点 + 前端"相似内容直达结果"区**：识别生成的检索词由服务器并行抓取 6 个免密钥公开数据源，**直接在页内展示真实网址**（缩略图 + 标题 + 可点击链接 + 来源），按 相似图片 / 相关视频 / 文章网页 / 百科词条 分组：
  - DuckDuckGo `i.js`（相似图片：托管页+图址）与 `v.js`（**相关视频：真实 B站/抖音/YouTube 链接含时长**，vqd 令牌两步获取并按查询缓存 10 分钟）；
  - Openverse API（CC 图库：图片 + Flickr/Wikimedia 托管页）；
  - 百度图片 `acjson`（缩略图 + 中文标题；其返回为非法 JSON，改用正则逐字段提取）；
  - 必应网页搜索（`u=a1` base64 重定向解码为真实文章 URL）；
  - 维基百科 REST 搜索（中文优先、英文兜底的词条 URL）。
- 识别完成自动触发直达检索；点击检索词 chip 即时切换关键词重新抓取；"用关键词框内容重新检索"按钮支持手动流程（不依赖模型）。
- 直达结果并入引用报告（新增第 3 节，按 GB/T 7714 要点逐条著录）与 JSON 导出。

### 变更

- 版本 1.2.0 → 1.3.0；README 更新特性、流程图（直达数据管道）、截图与 API 表。
