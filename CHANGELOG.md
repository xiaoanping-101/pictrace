...
## [1.4.0] — 2026-09-19（零点击直达：载图即出结果，新增社区帖子源）

### 新增

- **载图零点击自动直达检索**（主流程重构）：载入图片是唯一动作——语义模型曾启用的会话自动静默恢复（浏览器缓存权重，秒级）→ CLIP 自动识别取词 → `/api/discover` 自动抓取 → 页内直出真实网址；未启用模型时退回文件名/EXIF 线索取词，保证零点击路径始终可用。
- **社区帖子数据源**：Hacker News（Algolia 官方免密钥 API，实测稳定，返回真实帖子 URL+热度+作者）与 Reddit（`search.json`，部分数据中心 IP 被拒时自动降级）；直达结果新增"社区帖子"分组（v1.3 的四组扩为五组）。
- 直达检索升级为右列主卡片（"相似内容直达检索"+ 关键词即时重抓按钮）。

### 变更

- **引擎深链/聚合检索折叠进"高级"折叠区**（`<details>`）——工具定位从"聚合入口"转为"直接给结果"，深链仅作人工核验兜底；
- 数据源 6 → 8；版本 1.3.0 → 1.4.0；README 双语流程图与时序图同步（新增 alt/par 分支：模型静默恢复、帖子源并行）。
